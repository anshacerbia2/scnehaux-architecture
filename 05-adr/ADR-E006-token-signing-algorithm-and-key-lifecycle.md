...
doc_meta:
  id: ADR-E006
  title: ADR-E006 Token Signing Algorithm and Key Lifecycle Management
...
# ADR-E006: Token Signing Algorithm and Key Lifecycle Management
...
## 3. Context
The Scnehaux enterprise identity platform must issue cryptographically signed JSON Web Tokens (JWT) to represent authenticated identity across our distributed service landscape.
In high-throughput microservice request chains, every byte of the HTTP header counts. Large tokens increase network latency and memory allocation. ECDSA (`ES256`) signatures are highly compact (64 bytes in IEEE P1363 format) and fast to generate compared to RSA (`RS256`) signatures with 2048/3072-bit keys.
...
2.  **`Retiring`**: A new key has been promoted to `Active`. The retiring key is strictly prohibited from signing new tokens, but **remains advertised in the JWKS** to verify existing unexpired tokens. Keys remain in this state for the duration of the *Maximum Token TTL* (e.g., 24 hours for refresh tokens).
...
## 8. Risks
- **ECDSA Signature Encoding Conflicts**: Different programming languages parse ECDSA signatures differently. Some expect ASN.1 DER (common in OpenSSL/Go), others expect IEEE P1363 raw concatenation (common in JavaScript/OIDC standards).
  - *Mitigation*: Our token generator must translate the ASN.1 DER signature generated by Go's cryptographic package into a raw concatenated 64-byte sequence before attaching it to the final base64url JWT payload.
...
- [Identity Platform Domain Strategy (PAD-001)](file:///d:/Ansha/architecture-description/scnehaux-architecture/02-platform/identity-platform/identity-platform.pad.md)
...
- [Epoch-Based Session Management Decision (ADR-E004)](file:///d:/Ansha/architecture-description/scnehaux-architecture/04-decisions/ADR-E004-epoch-based-session-management.md)
