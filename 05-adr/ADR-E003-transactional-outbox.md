...
doc_meta:
  id: ADR-E003
  title: ADR-E003 Enterprise Transactional Outbox Pattern
...
# ADR-E003: Enterprise Transactional Outbox Pattern
...
## 8. Risks
- **Duplicate Event Delivery**: If a worker crashes mid-delivery or network timeouts occur during broker publish, events might be re-sent.
  - *Mitigation*: All downstream event consumers MUST be engineered to be completely idempotent using the unique `event_id` as a deduplication key.
...
- [Enterprise Application Architecture Strategy (EAD-003)](file:///d:/Ansha/architecture-description/scnehaux-architecture/01-enterprise/EAD-003-Application-Architecture.md)
