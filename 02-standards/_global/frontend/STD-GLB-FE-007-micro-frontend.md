...
- **Mandatory Alternative**: The bundle size limits may be exceeded *only* if the remote is guaranteed to not block the primary interactive shell's initialization (lazy-loaded), and objective profiler benchmarks prove that the host shell's INP and LCP remain unharmed.
