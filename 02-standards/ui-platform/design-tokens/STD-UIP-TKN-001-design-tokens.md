...
**Performance Constraint**: All motion tokens must resolve exclusively to `transform` and `opacity` CSS properties. Animating layout properties (`width`, `height`, `margin`) is prohibited under the Zero Layout Thrashing rule (see PAD-002, Section 5).
