...
from engine.parsing.markdown_ast import parse_date, clean_content_for_length, extract_section_contents, normalize_section, extract_links, get_section_order
...
def _validate_structure(v: BaseValidator) -> None:
    ...
    for section_name, section_text in sections_map.items():
        ...
        if len(clean_text) < min_length:
            v.add_error('stylistic_deviation', f"Section '{section_name}' content length ({len(clean_text)} chars) is below minimum of {min_length} chars.")
    ...
    for section_name, section_text in sections_map.items():
        if 'non-functional requirements' in section_name.lower():
            ...
            for header in sub_headers:
                ...
                matched = any(clean_header.lower() == p.lower() for p in aws_waf_pillars)
                if not matched:
                    v.add_error('structural_integrity_violation', f"NFR taxonomy violation: '{clean_header}' is not a recognized AWS WAF Pillar. Allowed pillars: {', '.join(aws_waf_pillars)}.")
...
def _validate_technology_hold(v: BaseValidator) -> None:
    ...
    tech_radar_path = os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', '..', '01-enterprise', 'tech-radar.yaml'))
