...
def audit_duplicate_ids(duplicate_ids: dict, severity_levels: dict) -> list[tuple[str, str, str]]:
    """
    Return a list of (severity, message, filepath) tuples for duplicate document IDs.
    """
    findings = []
...
def audit_hierarchy_tiers(all_doc_metadata: dict, severity_levels: dict) -> list[tuple[str, str, str]]:
    """
    ...
    for doc_id, meta in all_doc_metadata.items():
        ...
        if doc_id.startswith('TDD-'):
            ...
            if parent and not parent.startswith('SAD-'):
                findings.append((sev, f"Hierarchy violation: TDD '{doc_id}' must attach to a SAD, but parent_sad is '{parent}'.", filepath))
                
        # Check SAD
        elif doc_id.startswith('SAD-'):
            parent = meta.get('parent_pad')
            if parent and not parent.startswith('PAD-'):
                findings.append((sev, f"Hierarchy violation: SAD '{doc_id}' must attach to a PAD, but parent_pad is '{parent}'.", filepath))
        ...
        # Check PAD
            ...
            if parents and not has_ead and not any(p.startswith('GDC-') for p in parents):
                findings.append((sev, f"Hierarchy violation: PAD '{doc_id}' must attach to an EAD via governed_by.", filepath))

        # Check ADR / STD
        elif doc_id.startswith('ADR-') or doc_id.startswith('STD-'):
            parents = _as_list(meta.get('governed_by')) + _as_list(meta.get('parent_pad'))
            for p in parents:
                if not (p.startswith('EAD-') or p.startswith('PAD-') or p.startswith('GDC-')):
                    findings.append((sev, f"Hierarchy violation: {doc_id[:3]} '{doc_id}' cannot attach to '{p}'. Must be EAD or PAD.", filepath))
                    
    return findings

def audit_orphans(all_doc_metadata: dict, severity_levels: dict) -> list[tuple[str, str, str]]:
    """
    Ensure no orphans exist below EAD level.
    """
    findings = []
    ...
    for doc_id, meta in all_doc_metadata.items():
        ...
        if doc_id.startswith('TDD-') and not meta.get('parent_sad'):
            findings.append((sev, f"Orphan artifact: TDD '{doc_id}' must declare a 'parent_sad'.", filepath))
            
        elif doc_id.startswith('SAD-') and not meta.get('parent_pad'):
            findings.append((sev, f"Orphan artifact: SAD '{doc_id}' must declare a 'parent_pad'.", filepath))
            
        elif doc_id.startswith('PAD-') and not meta.get('governed_by'):
            findings.append((sev, f"Orphan artifact: PAD '{doc_id}' must declare 'governed_by' (pointing to an EAD).", filepath))
