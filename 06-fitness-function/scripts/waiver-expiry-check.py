...
def check_waiver_expiry():
    """
    Scan all accepted ADR Exceptions (waivers) for expiration against their declared `expiry_date`.
    Returns exit code 1 if any waivers have expired, breaking the CI/CD pipeline.
    """
                    ...
                    if adr_type == "exception" and status == "accepted":
                        ...
                        if expiry_date_raw:
                            ...
                            if expiry_date:
                                ...
                                if delta < 0:
                                    print(
                                        f"[CRITICAL] Expired waiver: {rel_path} expired on {expiry_date} ({abs(delta)} days ago)"
                                    )
                                    has_errors = True
                                elif delta <= warning_days:
                                    print(
                                        f"[WARNING] Expiring soon: {rel_path} expires in {delta} days on {expiry_date}"
                                    )
                            else:
                                print(
                                    f"[ERROR] Invalid expiry_date format in {file}: {expiry_date_raw}"
                                )
                                has_errors = True
