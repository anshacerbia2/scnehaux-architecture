...
from engine.validators.global_rules import _validate_inline_references
from engine.fs.crawler import resolve_registry_with_duplicates
from engine.auditors.graph_auditor import audit_traceability_graph
from engine.parsing.markdown_ast import strip_code_fences, extract_doc_id_references
from engine.cli import build_sarif

ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))


def _global_rules():
    with open(os.path.join(ROOT, '00-governance', 'schemas', 'base.schema.json'), encoding='utf-8') as f:
...
def _load_generator():
    gen_path = os.path.join(os.path.dirname(__file__), '..', 'generators', 'generate_rules_doc.py')
