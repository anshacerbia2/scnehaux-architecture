...
This guideline operationalizes the enforcement mechanisms mandated by:
- **[EAD-003 — Enterprise Application Architecture](../01-enterprise/EAD-003-application-architecture.md)**: Security boundaries (Zero Trust, mTLS), integration patterns, and PII isolation.
- **[EAD-004 — Enterprise Technology Architecture](../01-enterprise/EAD-004-technology-architecture.md)**: Container security (Trivy/SBOM), Kubernetes resource policies, and technology paved roads.
...
> **Project Convention**: `.semgrep/rules/` for custom rules; `.github/workflows/codeql.yml` for CodeQL.
...
### 2.2 Dependency Boundary Management (Dependency Cruiser)

> **Project Convention**: `.dependency-cruiser.js` at project root.

To ensure separation of concerns and prevent the "Big Ball of Mud" anti-pattern, architectural module boundaries must be enforced programmatically:
...
### 2.3 Policy-as-Code for Infrastructure (OPA Conftest)

> **Project Convention**: `policies/terraform/` and `policies/kubernetes/` directories.

Cloud infrastructure provisioning must be subjected to automated policy evaluations before deployment:
