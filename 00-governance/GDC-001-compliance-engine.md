...
3. **Traceability & Orphan Detection**: Enforces dependency links (e.g., a SAD must reference a valid `parent_pad` ID, an ADR must exist if an STD requires an exception).
4. **Content Constraints**: Scans the document body for prohibited vague vocabulary (e.g., "fast", "scalable") and enforces mandatory quantifiable metrics.
...
| **Content** | Prohibited Words | <ul><li>`maybe`</li><li>`probably`</li><li>`should consider`</li><li>`TBD`</li><li>`coming soon`</li><li>`and so on`</li><li>`easy`</li><li>`simple`</li><li>`just`</li><li>`basic`</li><li>`good`</li><li>`better`</li></ul> |
| **Content** | Ambiguity Check | **Pattern**: `(highly\|very) (scalable\|fast\|secure)`<br>**Message**: `Vague claim detected. Must be quantified with metrics.` |
| **Quantification** | Required For Sections | <ul><li>`Non-Functional Requirements`</li><li>`Observability Requirements`</li></ul> |
| **Quantification** | Metric Pattern | `\d+\s*(ms\|s\|%\|req/s\|GB\|TB\|MB\|PB\|QPS\|RPS\|RPM\|milliseconds)` |
...
| **Utilities** | `validators/utils.py` | **AST Processing**: Uses `markdown_it` to tokenize the markdown into an Abstract Syntax Tree (AST). Provides modular functions to accurately extract section content (`extract_section_contents`), strip styling for character counts (`clean_content_for_length`), harvest `href` links (`extract_links`), and normalize YAML `datetime` values (`parse_date`). |

### 2.3 Inline Policy Exemptions (`lint_disable`)

When a deviation is approved but the linter still blocks the CI pipeline, authors can bypass specific linter rules locally using HTML comments:

```html
<!-- lint_disable: vague_claim, structural_integrity_violation, prohibited_word -->
```
The use of `lint_disable` is strictly monitored, collected by the linter engine, and flagged in the final CI audit summary for Reviewer scrutiny.
