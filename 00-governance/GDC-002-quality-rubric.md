...
| :--- | :--- | :--- | :--- | :--- |
| **1** | **Clarity & Precision** | Eliminate "weasel words" and motivational filler. | Precise quantification (e.g., "System handles 5,000 RPS with < 200ms P99 latency"). | Using subjective adjectives like "highly available", "secure", "fast", or "scalable" without mathematical backing. |
...
1. **10 Binary (Pass/Fail) Checks vs. Weighted Individual Scoring (1-5)**
   - *Why rejected*: Grading *individual* parameters on a subjective 1-5 scale introduces negotiation between reviewer and author. Instead, we strictly enforce 10 binary (Pass/Fail) checks, which objectively sum up to a Total Score of 0-10.
