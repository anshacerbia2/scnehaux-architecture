...
## 3. Context
Following the decision to use React as the foundational rendering engine ([ADR-GLB-FE-001](../../../05-decisions/_global/frontend/ADR-GLB-FE-001-react-ecosystem.md)) and Rsbuild/Vite for SPAs ([ADR-GLB-FE-002](../../../05-decisions/_global/frontend/ADR-GLB-FE-002-build-toolchain.md)), the enterprise requires a standardized approach for applications that demand Server-Side Rendering (SSR), Search Engine Optimization (SEO), and full-stack API capabilities. Historically, teams have built custom Node.js Express servers to hydrate React, leading to severe architectural fragmentation, memory leaks, and complex deployment topologies.
...
- [ADR-GLB-FE-002 (Build Toolchains)](../../../05-decisions/_global/frontend/ADR-GLB-FE-002-build-toolchain.md)
