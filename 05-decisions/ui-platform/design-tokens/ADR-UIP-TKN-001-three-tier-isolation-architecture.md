...
1. **Semantic Ambiguity:** `$blue-9` provides no context on *why* the color is used (is it a primary button, an info alert, or a selected row?).
...
## 4. Decision Drivers
By enforcing this architectural boundary, we completely decouple the *Design Value* from the *Design Intent*. 
...
- **Developer Friction**: Developers might find dot-notation and multi-layered token resolution more complex than writing standard CSS.
  - *Mitigation*: Mitigated by providing comprehensive IDE autocomplete configurations and strongly-typed SCSS/TypeScript utility helpers.
