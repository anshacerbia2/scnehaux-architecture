...
def generate_index(directory, layer_name):
    ...
    for f in md_files:
        ...
        if meta and 'id' in meta:
            ...
            docs.append({
                ...
                'link': rel_path,
                'fulfilled_by': len(meta.get('fulfilled_by', [])) if meta.get('fulfilled_by') else 0,
    ...
    with open(index_path, 'w', encoding='utf-8') as f:
        ...
        f.write("> **Auto-generated**: This file is maintained by `scripts/generate_pad_sad_index.py`.\n\n")
        
        if layer_name == "Platform Architecture (PAD)":
            f.write("| ID | Title | Owner | Status | Fulfilled By (SADs) |\n")
            f.write("|---|---|---|---|---|\n")
            for d in docs:
                f.write(f"| [{d['id']}]({d['link']}) | {d['title']} | {d['owner']} | {d['status']} | {d['fulfilled_by']} |\n")
        else:
            ...
            for d in docs:
                ...
                if isinstance(parent, list):
                    parent = ", ".join(parent)
                f.write(f"| [{d['id']}]({d['link']}) | {d['title']} | {parent} | {d['owner']} | {d['status']} |\n")

    print(f"Generated {index_path}")

if __name__ == "__main__":
    base = "d:/Ansha/architecture-description/scnehaux-architecture"
