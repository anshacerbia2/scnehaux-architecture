...
- **A dead letter closes only on the consumer's own evidence.** It is resolved as `REPLAYED` when a delivery receipt shows the active projection consumer applied that exact event. It is resolved as `SUPERSEDED` when a receipt shows that consumer applied a newer version of the same Membership, with versions read from the producer's `membership.membership_event` history rather than stream positions. Either way the receipt carries `consumer_applied` evidence, which the consumer alone can produce. Operator assertion and scalar progress marks are not evidence. `TDD-organization-control-005` specifies the resolver.
- **A waiver is not a closure.** An incident refused by a consumer that has since been retired has no corrective path. It may be waived, with an author, a reason and an expiry within 90 days. The waiver silences the stale alert and lets retention dispose of the payload, and it never clears the debt. A waiver on the active consumer's incident, or on one that records no consumer, is refused.

Security debt is reported per consumer: the dispatcher records which consumer refused each event, and a consumer's frontier counts its own dead letters plus any that record no consumer. One consumer's poison event therefore does not refuse another consumer's traffic. The enforcement scope is still one producer and one projection consumer, because delivering one event to several consumers with separate outcomes needs a delivery substrate the outbox does not have. A second active projection consumer is refused until one is named.
...
| Superseded security event dead-lettered                  | newer version already applied, so a replay would be discarded; the operator resolves the dead letter as `SUPERSEDED` on the newer event's `consumer_applied` receipt | Every projection-backed check, estate-wide, until resolved                       |
