...
# HCM Business System Software Architecture (SAD-101)

> **Status: chartered.** The capability is chartered by PAD-BIZ-001 and no system is in build.
...
This document records the architectural constraints the HCM Business System inherits from the enterprise architecture. It is the placeholder that a full system design replaces.

### 1.1 Objective

Realize the capability PAD-BIZ-001 charters, within the constraints below, without re-implementing a capability another platform owns.

### 1.2 Capability

Human capital management: the Hire-to-Retire value stream.

### 1.3 Constraint

These are inherited and not open to per-system renegotiation:

- **One owning domain, one operational store.** This system owns the HCM Database and no other system reads it directly, per EAD-003 section 5.1.
...
The capability requirements are held by PAD-BIZ-001 until a system design exists. This document adds none of its own.
...
Consumes Identity, Workspace, Workflow, Notification, Document, and Audit as platform capabilities. It implements none of them.
...
The HCM Database holds employee records, HR organisational structure, and contracts. It is private to this domain: no other system holds a connection to it, and no cross-domain join exists, per EAD-003 sections 5.1 and 6.5.
...
Contained to this product. Other domains continue on their own stores and on cached read models of what this product has published.
...
#### 10.1 Implementing local authentication or a local user directory

Rejected outright by EAD-006 section 6.2. A product-local credential store was the weakest link in a recorded breach, and identity is consumed as a platform capability rather than re-implemented per product.
