...
# Document Platform Software Architecture (SAD-008)

> **Status: chartered.** The capability is chartered by PAD-PLT-009 and no system is in build.
...
This document records the architectural constraints the Document Platform inherits from the enterprise architecture. It is the placeholder that a full system design replaces.

### 1.1 Objective

Realize the capability PAD-PLT-009 charters, within the constraints below, without re-implementing a capability another platform owns.

### 1.2 Capability

File storage, document lifecycle, and rendering.

### 1.3 Constraint

These are inherited and not open to per-system renegotiation:

- **One owning domain, one operational store.** This system owns the Document Store and no other system reads it directly, per EAD-003 section 5.1.
...
The capability requirements are held by PAD-PLT-009 until a system design exists. This document adds none of its own.
...
Synchronous for retrieval and metadata, asynchronous for rendering and lifecycle transitions.
...
The Document Store holds binary objects and their metadata. It is private to this domain: no other system holds a connection to it, and no cross-domain join exists, per EAD-003 sections 5.1 and 6.5.
...
Retrieval fails for the affected objects while metadata reads continue from the database. Rendering is asynchronous, so a rendering outage delays derived artifacts without affecting stored originals.
...
#### 10.1 Storing binary content in the relational database

Rejected. Large objects in a relational store inflate backups, lengthen restore time, and place binary throughput on the same connection pool as transactional queries. Metadata is relational; content is object storage.
