...
doc_meta:
  id: SAD-005
  title: Notification Platform SAD
...
# Notification Platform Software Architecture (SAD-005)

> **Status: chartered.** The capability is chartered by PAD-PLT-005 and no system is in build.
...
This document records the architectural constraints the Notification Platform inherits from the enterprise architecture. It is the placeholder that a full system design replaces.

### 1.1 Objective

Realize the capability PAD-PLT-005 charters, within the constraints below, without re-implementing a capability another platform owns.

### 1.2 Capability

Multi-channel delivery: email, SMS, push, and webhook.

### 1.3 Constraint

These are inherited and not open to per-system renegotiation:

- **One owning domain, one operational store.** This system owns the Notification Database and no other system reads it directly, per EAD-003 section 5.1.
...
The capability requirements are held by PAD-PLT-005 until a system design exists. This document adds none of its own.
...
Asynchronous delivery. Business Products publish an event; this platform delivers.
...
The Notification Database holds delivery state, templates, and per-channel provider configuration. It is private to this domain: no other system holds a connection to it, and no cross-domain join exists, per EAD-003 sections 5.1 and 6.5.
...
Delivery is delayed rather than lost. Undelivered messages remain in the outbox and send on recovery; a failing channel provider is isolated by its own circuit breaker so one provider cannot exhaust the others.
...
#### 10.1 Synchronous send on the caller's request path

Rejected. A caller blocked on an email provider inherits that provider's latency and availability, and a provider outage becomes the caller's outage. Delivery is an asynchronous consequence of a business event, so it is triggered by an event.
