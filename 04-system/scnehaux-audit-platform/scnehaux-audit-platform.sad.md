...
# Audit Platform Software Architecture (SAD-010)

> **Status: chartered.** The capability is chartered by PAD-PLT-007 and no system is in build.
...
This document records the architectural constraints the Audit Platform inherits from the enterprise architecture. It is the placeholder that a full system design replaces.

### 1.1 Objective

Realize the capability PAD-PLT-007 charters, within the constraints below, without re-implementing a capability another platform owns.
...
- **One owning domain, one operational store.** This system owns the Audit Ledger and no other system reads it directly, per EAD-003 section 5.1.
...
The capability requirements are held by PAD-PLT-007 until a system design exists. This document adds none of its own.
...
Asynchronous and fire-and-forget. Every domain publishes evidence; this platform is never on a caller's critical path.
...
The Audit Ledger holds append-only, tamper-evident evidence records. It is private to this domain: no other system holds a connection to it, and no cross-domain join exists, per EAD-003 sections 5.1 and 6.5.
...
Evidence is delayed, never dropped. Producers write to a local durable queue and backfill on recovery. Loss of the audit path fails security-critical writes closed at the producer, per EAD-006 section 8.
...
#### 10.1 Mutable audit records with an update path

Rejected. An audit trail that can be altered is evidence of nothing, because an investigation cannot distinguish an absent record from a removed one. Records are append-only and hash-chained so retro-active modification is detectable.
